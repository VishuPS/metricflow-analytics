...
Password reset links are generated by the Worker and must be delivered by an email provider. Configure an email webhook before enabling resets for real users:
...
The Worker posts `{ type, to, name, resetUrl, subject }` to that webhook. For local/debug-only testing, set `PASSWORD_RESET_DEBUG_LINK=true` to include the reset URL in the API response; do not enable that in production.
